 ...
 * time, not the visitor's — the point is that there is a real person on the
 * other end of this right now, not a borrowed timezone.
 *
 * Ticks once a minute. A seconds display would be a per-second re-render of the
 * header for a detail nobody reads.
 */
function LiveStatus() {
  const [now, setNow] = useState(() => founderTime())

  useEffect(() => {
    const id = window.setInterval(() => setNow(founderTime()), 60_000)
    return () => window.clearInterval(id)
  }, [])

  return (
    <p
      className="pointer-events-none hidden items-center gap-2.5 font-mono text-[0.66rem] max-sm:text-[0.75rem] tracking-[0.16em] text-faint uppercase xl:flex"
      aria-hidden="true"
    >
  ...
  return (
    <>
      ...
      >
          ...
          <div className="flex items-center gap-7">
            ...
            <div className="relative hidden md:block" ref={menu}>
              <button
                ...
                aria-haspopup="true"
                className="group flex items-center gap-1.5 font-mono text-[0.7rem] max-sm:text-[0.75rem] tracking-[0.12em] text-muted uppercase transition-colors hover:text-ink"
                data-cursor-target
              >
                Menu
                <svg
        ...
        {/*
          The sheet is a sibling of the header on purpose, not a child. Once the
          page scrolls, the header gets `backdrop-blur`, and a backdrop-filter
          makes its element the containing block for `position: fixed`
          descendants. Inside the header this panel was sized against the 72px
          bar instead of the screen, collapsing to about 1px tall while the
          body was locked: the menu vanished and the page would not scroll.
        */}
        {/*
        ...
        */}
        <div
          ...
          id="mobile-nav"
          data-lenis-prevent
          className={`fixed inset-x-0 top-[4.5rem] bottom-0 z-40 overflow-y-auto overscroll-contain border-t border-line bg-void/98 backdrop-blur-2xl transition-opacity duration-300 md:hidden ${
            open ? 'opacity-100' : 'pointer-events-none opacity-0'
          }`}
        >
          <ul className="wrap flex flex-col gap-1 pt-8 pb-[max(2rem,env(safe-area-inset-bottom))]">
            {MENU_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
