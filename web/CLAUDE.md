...
keyframes in the site palette, no video or image asset. The overlay fades
*itself* out via CSS; the React node then unmounts on a later timer, invisibly.
`App` keeps the 3D scene unmounted until the intro fires `INTRO_DONE_EVENT` at
`VISUAL_MS` — three.js init is a heavy main-thread block that, run under the
intro, stalls its own animation and stretches ~1s past 2s (the chunk still
downloads meanwhile). Runs once per browser session (`sessionStorage`), never
